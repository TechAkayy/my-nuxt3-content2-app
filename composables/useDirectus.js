...
export async function useDirectus() {
	...
	if (!directus.auth.token) await authenticate(directus)

	return directus
}

async function authenticate(directus) {
	if (
		import.meta.env.VITE_DIRECTUS_EMAIL &&
		import.meta.env.VITE_DIRECTUS_PASSWORD
	) {
